...
int main() {
  ...
  /* Select color subroutine */
  const auto colorEffectSubroutineLocation = glGetSubroutineUniformLocation(
      program, GL_FRAGMENT_SHADER, "colorEffect");

  const auto noColorEffectSubroutineIndex =
      glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "noColorEffect");
  const auto invertColorEffectSubroutineIndex =
      glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "invertColor");

  int maxSubroutineUniformLocations;
  glGetIntegerv(GL_MAX_SUBROUTINE_UNIFORM_LOCATIONS,
                &maxSubroutineUniformLocations);

  auto subroutineIndices = new unsigned int[maxSubroutineUniformLocations];
  subroutineIndices[colorEffectSubroutineLocation] =
      invertColorEffectSubroutineIndex;
  glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, maxSubroutineUniformLocations,
                          subroutineIndices);
