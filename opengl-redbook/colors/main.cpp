...
static auto GetSquareBufferObjects() {
  ...
  const float vertices[] = {
      // positions          // colors
      -0.9f, 0.9f,  1.0f, 0.0f, 0.0f,  // top left
      0.9f,  0.9f,  0.0f, 1.0f, 0.0f,  // top right
      -0.9f, -0.9f, 0.0f, 0.0f, 1.0f,  // bottom left
      0.9f,  -0.9f, 1.0f, 1.0f, 0.0f   // bottom right
  };
...
static auto GetSquareVAO(const unsigned int program) {
  ...
  glVertexAttribPointer(position_loc, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 5,
                        nullptr);
  glVertexAttribPointer(color_loc, 3, GL_FLOAT, GL_FALSE, sizeof(float) * 5,
                        (void*)(sizeof(float) * 2));
