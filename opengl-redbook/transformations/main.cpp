...
int main() {
  ...
  auto transform_location = glGetUniformLocation(program, "mTransform");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glm::mat4 trans_matrix = glm::mat4(1.0);
    trans_matrix = glm::rotate(trans_matrix, (float)glfwGetTime(),
    ...
    trans_matrix = glm::translate(trans_matrix, glm::vec3(-0.5, 0.5, 0.0));
    auto scalar = static_cast<float>(glm::sin(glfwGetTime()));
    trans_matrix = glm::scale(trans_matrix, glm::vec3(scalar, scalar, 1.0));
    glUniformMatrix4fv(transform_location, 1, GL_FALSE,
                       glm::value_ptr(trans_matrix));
